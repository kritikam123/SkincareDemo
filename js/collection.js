...
const sortResult = document.getElementById("sort-select");

//product cards for list of products
function renderProducts(list) {
    ...
    resultsCountEl.textContent = `Showing ${list.length} product${list.length > 1 ? "s" : ""}`;
...
//If any of these products are already in the cart (from a previous visit), show "added" instead id add to cart right away
...
function applyFilterAndSort() {
    const checkedCategories = Array.from(document.querySelector(".filter-category:checked")).map(el => el.value);
    const checkedConcerns = Array.from(document.querySelector(".filter-concern:checked")).map(el => el.value);
    const minPrice = parseFloat(document.getElementById("price-min").value) || 0;
    const maxPrice = parseFloat(document.getElementById("price-max").value) || Infinity;

    let filtered = productsContainer.filter(product => {
        const matchesCategory = checkedCategories.length === 0 || checkedCategories.includes(product.category);
        const matchConcern = checkedConcerns.length === 0 || checkeedConverns.includes(product.concern);
        const matchesPrice = product.price >= minPrice && product.price <= maxPrice;
        return matchesCategory && matchConcern && matchesPrice;
    });
...
document.querySelectorAll(".filter-cartegory, .filter-concern").forEach(checkbox =>{
    checkbox.addEventListener("change", applyFiltersSort);
});
